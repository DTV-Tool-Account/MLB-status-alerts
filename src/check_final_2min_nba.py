# v1: NBA Final 2 Minutes Monitor
import os
import requests
...
print("🚀 NBA Script starting...", flush=True)
...
ALERTED_GAMES_FILE = "alerted_games_nba.pkl"
...
def is_final_2min(status_text, quarter):
    """Check if game is in final 2 minutes of 4th quarter"""
    try:
        if quarter != 4:
            return False
        ...
        # Check for final 2 minutes indicator
        if ':' in status_text:
            ...
            if len(parts) == 2:
                try:
                    minutes = int(parts[0])
                    seconds = int(parts[1])
                    # Final 2 minutes is between 2:00 and 0:00
...
def get_nba_games():
    ...
    print("📡 Fetching NBA games from ESPN API...", flush=True)
    try:
        ...
        print(f"❌ Failed to fetch NBA games: {e}", flush=True)
        return []

def check_final_2min_games():
    print("📋 Starting NBA check...", flush=True)
    try:
        ...
        if not events:
            print("ℹ️ No NBA games found", flush=True)
            return
        ...
        print(f"✅ NBA check complete!", flush=True)
